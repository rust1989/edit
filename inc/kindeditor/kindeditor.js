...
KE.plugin['attachment'] = {
	click : function(id) {
		try{
			openUploadAttach('editor',id);
		}catch(err){
			alert('openUploadAttach() undefined!');			
		}
	}
};
...
KE.plugin['image'] = {
	...
	check : function(id) {
		...
		if (!urlBox.value.match(/\.(jpg|jpeg|gif|bmp|png)(\s|\?|$)/i) && !urlBox.value.match(/attachment\.php(\s|\?|$)/i)) {
