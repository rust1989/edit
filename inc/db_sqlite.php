...
class db {
	...
	//发送SQL 查询，并返回结果集
    function query($query_id, $query_type='sqlite_query'){
		$query_id=preg_replace('/`'.$this->pre.'(.[^`]*)`/i','\''.$this->pre.'\1\'',$query_id);
        $this->query_id = $query_type($this->conn, $query_id);
	...
	function query_unbuffered($sql=""){
		return $this->query($sql, 'sqlite_unbuffered_query');
	}
	//从结果集中取得一行作为关联数组
	...
	//关闭 SQLite 连接
    function close_db(){
    	if ( $this->conn ) return @sqlite_close( $this->conn );
    }
    ...
    function halt($the_error=""){
		...
		if ($this->debug==true){
			echo "<html><head><title>SQLite Error</title><meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\" />";
